...
import Tick1 from '../../../../assets/imge/Register-imge/tick-circle.svg'
const Driving_license = () => {
    const [fileNames, setFileNames] = useState<any>([]);
    const [size, setSize] = useState<any>([]);
    const [fileSelected, setFileSelected] = useState<boolean>(false);
    const [icon, setIcon] = useState(<Trash1 width={30} height={30} />);
    ...
    useEffect(() => {
        if (progress === 1) {
            setIcon(<Tick1 width={30} height={30} />);
            const timeoutId = setTimeout(() => {
                setIcon(<Trash1 width={30} height={30} />);
            }, 2000);
